  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setError(res.error || "Login failed. Admin credentials incorrect.");
      }
    } catch (e) {
      ...
      <main className="flex-grow bg-background py-16 flex items-center justify-center">
        <div className="w-full max-w-md px-4 space-y-6">
          ...
          <div className="text-center space-y-2">
            ...
            <h1 className="text-2xl font-black text-foreground">Admin Portal Sign In</h1>
            <p className="text-xs text-muted-foreground">Authenticate to review pending provider documents, edit service schemas, and pull export files.</p>
          </div>
          ...
          <div className="bg-card border border-border p-6 sm:p-8 rounded-3xl shadow-md space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="block text-3xs font-semibold uppercase text-muted-foreground">Admin Email Address</label>
            ...
            <div className="border-t border-border pt-4 text-3xs space-y-1 text-muted-foreground bg-secondary/20 p-3 rounded-lg">
              ...
              <p>Email: <span className="font-bold select-all text-primary">admin@test.com</span></p>
              <p>Password: <span className="font-bold select-all text-primary">adminpassword</span></p>
            </div>
